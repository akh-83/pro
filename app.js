...
const tr = {
  ...
  'Eng': {
    ...
    btns: { calc:'Calculate', info:'Instruction', cancel:'Cancel', done:'Done', pick:'Time', saveRes:'Save Result', search:'Search', skip:'Skip' },
  ...
  'Pol': {
    ...
    btns: { calc:'Oblicz', info:'Instrukcja', cancel:'Anuluj', done:'Gotowe', pick:'Czas', saveRes:'Zapisz wynik', search:'Szukaj', skip:'Pomiń' },
    ...
    admin: { title: 'Panel administratora', back: 'Wyjdź z profilu', users: 'Użytkownicy', resetData: 'Zresetuj dane', resetConfirm: 'Usunąć WSZYSTKIE dane lokalne?', resetSuccess: 'Dane zresetowane!', deleteAll: 'Usuń wszystkich', deleteUser: 'Usuń', deleteConfirm: 'Na pewno usunąć?' },
  ...
  'Esp': {
    ...
    btns: { calc:'Calcular', info:'Info', cancel:'Cancelar', done:'Hecho', pick:'Tiempo', saveRes:'Guardar result', search:'Buscar', skip:'Omitir' },
    ...
    auth: { title: 'Autorización', desc: 'Ingrese su número de trabajo', btn: 'Iniciar sesión', confirmWorker: 'Confirme el número: ' },
    admin: { title: 'Panel de administrador', back: 'Salir', users: 'Usuarios', resetData: 'Restablecer datos', resetConfirm: '¿Eliminar TODOS los datos locales?', resetSuccess: '¡Datos restablecidos!', deleteAll: 'Eliminar todos', deleteUser: 'Eliminar', deleteConfirm: '¿Estás seguro?' },
  ...
  'Ukr': {
    ...
    btns: { calc:'Розрахувати', info:'Інфо', cancel:'Відміна', done:'Готово', pick:'Час', saveRes:'Зберегти результат', search:'Пошук', skip:'Пропустити' },
    ...
    auth: { title: 'Авторизація', desc: 'Введіть робочий номер', btn: 'Увійти', confirmWorker: 'Підтвердіть робочий номер: ' },
    admin: { title: 'Панель Адміністратора', back: 'Вийти з профілю', users: 'Користувачі', resetData: 'Скинути дані', resetConfirm: 'Видалити ВСІ локальні дані?', resetSuccess: 'Дані скинуто!', deleteAll: 'Видалити всіх', deleteUser: 'Видалити', deleteConfirm: 'Ви впевнені?' },
...
function updateHeader() {
    ...
    if(document.getElementById('profileName')) document.getElementById('profileName').textContent = nameToDisplay;
}
...
async function checkAuth() {
    ...
    if (document.getElementById('globalUserCount')) {
    ...
    if (res && res.status === 'success') {
        ...
        if (res.total_users !== undefined && document.getElementById('globalUserCount')) {
            document.getElementById('globalUserCount').textContent = "users: " + res.total_users;
        }
    } else {
        if (document.getElementById('globalUserCount')) {
            document.getElementById('globalUserCount').textContent = "Ошибка сети/БД";
        }
    }
...
async function saveWorkerId() {
    ...
    if (res && res.total_users !== undefined && document.getElementById('globalUserCount')) {
        document.getElementById('globalUserCount').textContent = "users: " + res.total_users;
    }
...
async function initAppData() {
    ...
    let prevCountText = "users: 0";
    if (document.getElementById('globalUserCount')) {
        prevCountText = document.getElementById('globalUserCount').textContent;
    ...
    if (res && res.status === 'success') {
         ...
         rcpState = null;
         if (document.getElementById('globalUserCount')) {
             document.getElementById('globalUserCount').textContent = "Ошибка подключения";
         }
         alert("Не удалось загрузить данные с сервера. Пожалуйста, проверьте подключение или базу данных Supabase.");
    }
...
function afterDataLoad() {
    if (isAdmin && viewingWorkerId) {
        if (document.getElementById('globalUserCount')) document.getElementById('globalUserCount').textContent = "Admin Mode";
    }
    updateHeader();
    checkRCP();
    openProfile();
...
function checkRCP() {
    ...
    if (!rcpState || rcpState.dateStr !== today) {
        if(document.getElementById('rcpTodayDate')) document.getElementById('rcpTodayDate').textContent = today;
...
function renderRcpTab() {
    ...
    if(document.getElementById('rcpMonthLabel')) document.getElementById('rcpMonthLabel').textContent = t.months[m] + " " + y;
    ...
    let whHtml = 
        ...
        "</div>";
    if(document.getElementById('rcpTabHours')) document.getElementById('rcpTabHours').innerHTML = whHtml;
    ...
    calHtml += "</div>";
    if(document.getElementById('rcpTabCalendar')) document.getElementById('rcpTabCalendar').innerHTML = calHtml;
    
    if (!cardsHtml) cardsHtml = `<div style='text-align:center; color:#8b949e; margin-top:20px;'>${t.dash.noData}</div>`;
    if(document.getElementById('rcpTabCards')) document.getElementById('rcpTabCards').innerHTML = cardsHtml;
}
...
function openSaveModal(isEdit, idx) {
    ...
    if (wTypeSelect) wTypeSelect.value = rec.wTypeKey || 't1';
...
function setLang(lang) {
  ...
  if(document.getElementById('l_qty')) document.getElementById('l_qty').textContent = t.labels.qty;
  if(document.getElementById('l_tpi')) document.getElementById('l_tpi').textContent = t.labels.tpi;
  if(document.getElementById('l_prep')) document.getElementById('l_prep').textContent = t.labels.prep;
  if(document.getElementById('l_prepSub')) document.getElementById('l_prepSub').textContent = t.labels.prepSub;
  if(document.getElementById('l_start')) document.getElementById('l_start').textContent = t.labels.start;
  if(document.getElementById('l_end')) document.getElementById('l_end').textContent = t.labels.end;
  if(document.getElementById('b_calc')) document.getElementById('b_calc').textContent = t.btns.calc;
  if(document.getElementById('p_cancel')) document.getElementById('p_cancel').textContent = t.btns.cancel;
  if(document.getElementById('p_done')) document.getElementById('p_done').textContent = t.btns.done;
  if(document.getElementById('p_title')) document.getElementById('p_title').textContent = t.btns.pick;
  ...
  if(document.getElementById('rcp_btn_skip')) document.getElementById('rcp_btn_skip').textContent = t.btns.skip || 'Пропустить';
  ...
  if(document.getElementById('s_title')) document.getElementById('s_title').textContent = t.saveModal.title;
  ...
  if(document.getElementById('s_stan')) document.getElementById('s_stan').textContent = t.saveModal.stan;
  if(document.getElementById('s_type')) document.getElementById('s_type').textContent = t.saveModal.type;
  if(document.getElementById('s_uwagi')) document.getElementById('s_uwagi').textContent = t.workTime.uwagi;
  if(document.getElementById('s_btn_save')) document.getElementById('s_btn_save').textContent = t.saveModal.btnSave;
  if(document.getElementById('s_btn_cancel')) document.getElementById('s_btn_cancel').textContent = t.saveModal.btnCancel;
  if(document.getElementById('s_date')) document.getElementById('s_date').textContent = t.saveModal.date || 'Дата';

  if(document.getElementById('i_title_info')) document.getElementById('i_title_info').textContent = t.btns.info;
  if(document.getElementById('t_hist_title')) document.getElementById('t_hist_title').textContent = t.dash.hist;
  if(document.getElementById('b_search')) document.getElementById('b_search').textContent = t.btns.search;

  if(document.getElementById('re_title')) document.getElementById('re_title').textContent = t.editRcp;
  if(document.getElementById('re_l_start')) document.getElementById('re_l_start').textContent = t.reStart;
  if(document.getElementById('re_l_end')) document.getElementById('re_l_end').textContent = t.reEnd;
  if(document.getElementById('re_btn_del')) document.getElementById('re_btn_del').textContent = t.del;
  ...
  if(document.getElementById('t_rcp_title')) document.getElementById('t_rcp_title').textContent = t.rcpTabTitle;
  ...
  if(document.getElementById('instructionContentNorm')) document.getElementById('instructionContentNorm').innerHTML = htmlNorm;
  ...
  if(document.getElementById('result') && document.getElementById('result').innerHTML !== "") calculate();
}
...
function updatePrepStatus() {
  const t = tr[currentLang];
  if(document.getElementById('radioDot')) document.getElementById('radioDot').style.display = isPrepActive ? 'block' : 'none';
  if(isPrepActive) {
      if(document.getElementById('prepToggle')) document.getElementById('prepToggle').classList.add('active');
  } else {
      if(document.getElementById('prepToggle')) document.getElementById('prepToggle').classList.remove('active');
  }
