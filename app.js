...
var userFirstName = "Пользователь";
if (tgApp && tgApp.initDataUnsafe && tgApp.initDataUnsafe.user) {
    userFirstName = tgApp.initDataUnsafe.user.first_name;
...
const tr = {
  'Pyc': {
    ...
    admin: { title: 'Панель Администратора', back: 'Выйти из профиля', users: 'Пользователи', resetData: 'Сбросить локальные данные', resetConfirm: 'Вы уверены, что хотите удалить ВСЕ локальные данные из Telegram и браузера? Это действие необратимо!', resetSuccess: 'Данные успешно сброшены! Перезапуск...', deleteAll: 'Удалить всех', deleteUser: 'Удалить пользователя', deleteConfirm: 'Вы уверены, что хотите удалить эти данные? Это действие необратимо!' }
  },
  'Eng': {
    ...
    admin: { title: 'Admin Panel', back: 'Exit profile', users: 'Users', resetData: 'Reset local data', resetConfirm: 'Are you sure you want to delete ALL local data from Telegram and browser? This action is irreversible!', resetSuccess: 'Data successfully reset! Restarting...', deleteAll: 'Delete all', deleteUser: 'Delete user', deleteConfirm: 'Are you sure you want to delete these data? This action is irreversible!' }
  },
  'Pol': {
    ...
    admin: { title: 'Panel administratora', back: 'Wyjdź z profilu', users: 'Użytkownicy', resetData: 'Zresetuj dane lokalne', resetConfirm: 'Czy na pewno chcesz usunąć WSZYSTKIE dane lokalne z Telegrama i przeglądarki? Ta akcja jest nieodwracalna!', resetSuccess: 'Dane pomyślnie zresetowane! Ponowne uruchamianie...', deleteAll: 'Usuń wszystkich', deleteUser: 'Usuń użytkownika', deleteConfirm: 'Czy na pewno chcesz usunąć te dane? Ta akcja jest nieodwracalna!' }
  },
  'Esp': {
    ...
    admin: { title: 'Panel de administrador', back: 'Salir del perfil', users: 'Usuarios', resetData: 'Restablecer datos locales', resetConfirm: '¿Está seguro de que desea eliminar TODOS los datos locales de Telegram y del navegador? ¡Esta acción es irreversible!', resetSuccess: '¡Datos restablecidos con éxito! Reiniciando...', deleteAll: 'Eliminar todos', deleteUser: 'Eliminar usuario', deleteConfirm: '¿Estás seguro de que quieres eliminar estos datos? ¡Esta acción es irreversible!' }
  },
  'Ukr': {
    ...
    admin: { title: 'Панель Адміністратора', back: 'Вийти з профілю', users: 'Користувачі', resetData: 'Скинути локальні дані', resetConfirm: 'Ви впевнені, що хочете видалити ВСІ локальні дані з Telegram та браузера? Ця дія незворотня!', resetSuccess: 'Дані успішно скинуто! Перезапуск...', deleteAll: 'Видалити всіх', deleteUser: 'Видалити користувача', deleteConfirm: 'Ви впевнені, що хочете видалити ці дані? Ця дія незворотня!' }
  }
};
...
async function apiCall(action, payload = {}) {
    ...
    const uname = tgApp?.initDataUnsafe?.user?.first_name || 'User';
...
function checkRCP() {
    ...
    if (!rcpState || rcpState.dateStr !== today) {
        ...
        if(document.getElementById('rcpWorkerInput')) document.getElementById('rcpWorkerInput').value = currentWorkerId || '';
...
function confirmRCPStart() {
    ...
    const wNum = document.getElementById('rcpWorkerInput') ? document.getElementById('rcpWorkerInput').value : currentWorkerId;
...
function renderRcpTab() {
    ...
    for(let i=1; i<=daysInMonth; i++) {
        ...
        if (hasData) {
            ...
            cardsHtml += `<div class='hist-card' style='margin-bottom:8px; cursor:pointer;' onclick='openRcpEdit("${checkDateStr}")'>
        ...
        calHtml += `<div class='${dayClass}' style='${dayStyle}; cursor:pointer;' onclick='openRcpEdit("${checkDateStr}")'>${i}</div>`;
    }
    calHtml += "</div>";
...
function renderHistoryCards(searchText) {
    ...
    savedHistory.forEach(function(item, idx) {
        ...
        let modernBtns = "<div class='hc-actions' style='margin:0; display:flex; gap:8px; align-items:center;'>" +
...
async function loadAdminUsers() {
    ...
    if(res && res.users && res.users.length > 0) {
        ...
        // Fallback for visual test if no DB
        adminUsersList = ['12345', '98765', '45678'];
    }
    ...
    uHtml += "<p style='color:#8b949e; margin-top:0;'>" + t.admin.users + ": " + adminUsersList.length + "</p><div class='admin-grid' style='width:100%; box-sizing:border-box;'>";
    
    adminUsersList.forEach(u => {
        uHtml += "<div class='admin-card' onclick='viewUser(\"" + u + "\")'>Worker " + u + "</div>";
    });
    uHtml += "</div>";
...
function openProfile() {
    ...
    let exitBtn = "";
    if (isAdmin && viewingWorkerId) {
        exitBtn = "<div style='display:flex; gap:10px; margin-bottom:15px; width: 100%; box-sizing: border-box;'>" +
                  "<button class='btn btn-info' style='flex:1; font-size:14px; padding:10px;' onclick='exitAdminView()'>" + t.admin.back + "</button>" +
                  "<button class='btn btn-info' style='flex:1; background:#f06767; border-color:#f06767; font-size:14px; padding:10px;' onclick='adminDeleteUser(\"" + viewingWorkerId + "\")'>" + t.admin.deleteUser + "</button>" +
                  "</div>";
    }
