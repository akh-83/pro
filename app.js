...
const tr = {
  'Pyc': {
    ...
    admin: { title: 'Панель Администратора', back: 'Выйти из профиля', users: 'Пользователи', resetData: 'Сбросить локальные данные', resetConfirm: 'Вы уверены, что хотите удалить ВСЕ локальные данные из Telegram и браузера? Это действие необратимо!', resetSuccess: 'Данные успешно сброшены! Перезапуск...' }
  },
  'Eng': {
    ...
    admin: { title: 'Admin Panel', back: 'Exit profile', users: 'Users', resetData: 'Reset local data', resetConfirm: 'Are you sure you want to delete ALL local data from Telegram and browser? This action is irreversible!', resetSuccess: 'Data successfully reset! Restarting...' }
  },
  'Pol': {
    ...
    admin: { title: 'Panel administratora', back: 'Wyjdź z profilu', users: 'Użytkownicy', resetData: 'Zresetuj dane lokalne', resetConfirm: 'Czy na pewno chcesz usunąć WSZYSTKIE dane lokalne z Telegrama i przeglądarki? Ta akcja jest nieodwracalna!', resetSuccess: 'Dane pomyślnie zresetowane! Ponowne uruchamianie...' }
  },
  'Esp': {
    ...
    admin: { title: 'Panel de administrador', back: 'Salir del perfil', users: 'Usuarios', resetData: 'Restablecer datos locales', resetConfirm: '¿Está seguro de que desea eliminar TODOS los datos locales de Telegram y del navegador? ¡Esta acción es irreversible!', resetSuccess: '¡Datos restablecidos con éxito! Reiniciando...' }
  },
  'Ukr': {
    ...
    admin: { title: 'Панель Адміністратора', back: 'Вийти з профілю', users: 'Користувачі', resetData: 'Скинути локальні дані', resetConfirm: 'Ви впевнені, що хочете видалити ВСІ локальні дані з Telegram та браузера? Ця дія незворотня!', resetSuccess: 'Дані успішно скинуто! Перезапуск...' }
  }
};
...
async function loadAdminUsers() {
    ...
    uHtml += "<button class='btn btn-info' style='background:#f06767; border-color:#f06767; width:100%; margin-bottom:15px;' onclick='resetAllLocalData()'>" + t.admin.resetData + "</button>";
    uHtml += "<p style='color:#8b949e; margin-top:0;'>" + t.admin.users + ": " + adminUsersList.length + "</p><div class='admin-grid'>";
...
function openProfile() {
    ...
    let exitBtn = isAdmin && viewingWorkerId ? "<button class='btn btn-info' style='width:100%; margin-bottom:15px;' onclick='exitAdminView()'>" + t.admin.back + "</button>" : "";
