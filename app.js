...
async function apiCall(action, payload = {}) {
    ...
    try {
        if (action === 'getUserData') {
            ...
            // PGRST116 означает "0 строк" (юзер новый), это нормально. Остальные ошибки выводим:
            if (error && error.code !== 'PGRST116') alert("Ошибка БД (Загрузка юзера): " + error.message);
        ...
        if (action === 'getData') {
            ...
            const { data: userData, error: uErr } = await supabase.from('workers').select('*').eq('worker_id', targetWid).single();
            ...
            if (hErr) alert("Ошибка БД (История): " + hErr.message);
        ...
        if (action === 'saveHistory') {
            ...
            if (records.length > 0) {
                ...
                if (error) alert("Ошибка БД (Запись нормы): " + error.message);
            }
        ...
        if (action === 'saveRCP') {
            ...
            if (insertData.length > 0) {
                ...
                if (error) alert("Ошибка БД (Запись времени): " + error.message);
            }
            return { status: 'success' };
        }
        ...
        if (action === 'updateProfile') {
            ...
            if (error) alert("Ошибка БД (Обновление профиля): " + error.message);
        ...
        if (action === 'getAllUsers') {
            ...
            if (error) alert("Ошибка БД (Список юзеров): " + error.message);
        ...
        alert("КРИТИЧЕСКАЯ ОШИБКА JS: " + e.toString());
