...
const tr = {
  'Pyc': {
    ...
    instrNorm: [
        ...
        {t: 'История роботи', c: 'Список всех ваших сохраненных расчетов. Вы можете искать по PRO, редактировать и удалять записи.'},
...
// === API SUPABASE (ЗАМЕНИЛ СТАРЫЙ GOOGLE FETCH) ===
async function apiCall(action, payload = {}) {
    ...
    try {
        ...
        if (action === 'getData') {
            ...
            const { data: histData } = await supabase.from('pro_history').select('*').eq('worker_id', targetWid).order('created_at', { ascending: false });
            const { data: rcpData } = await supabase.from('time_history').select('*').eq('worker_id', targetWid).order('created_at', { ascending: true });
        ...
        if (action === 'saveHistory') {
            ...
            if (records.length > 0) {
                ...
                await supabase.from('pro_history').insert(insertData);
            }
        ...
        if (action === 'saveRCP') {
            ...
            if (insertData.length > 0) {
                await supabase.from('time_history').insert(insertData);
            }
            return { status: 'success' };
        }
