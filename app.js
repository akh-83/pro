...
async function apiCall(action, payload = {}) {
    ...
    try {
        ...
        if (action === 'registerUser') {
            // Upsert: Создает нового или обновляет существующего по tg_id
        ...
        // --- 3. ПОЛУЧЕНИЕ ВСЕЙ ИСТОРИИ (НОРМА + RCP) ---
        if (action === 'getData') {
            ...
            // Берем историю расчетов (сортировка от новых к старым)
            ...
            // Собираем историю расчетов обратно в формат, который понимает твой фронтенд
            ...
            // Собираем RCP обратно (разделяем на закрытые смены и текущую открытую)
            let rcpHistoryList = [];
        ...
        if (action === 'saveHistory') {
            ...
            // Поскольку фронтенд шлет весь массив целиком (при добавлении, изменении или удалении),
            // мы удаляем старые записи этого юзера и заливаем свежий отсортированный массив. 
        ...
        // --- 5. СОХРАНЕНИЕ РАБОЧЕГО ВРЕМЕНИ (RCP) ---
        if (action === 'saveRCP') {
            ...
            // Если есть открытая смена прямо сейчас
            if (payload.state) {
                insertData.push({
                    worker_id: wid,
        ...
        if (action === 'updateProfile') {
            ...
            // Если сменился номер, переносим всю его историю на новый номер
        ...
        if (action === 'deleteAllUsers') {
            if (uid.toString() !== ADMIN_ID) return { status: 'error' };
            // Удаляем всех кроме самого админа
